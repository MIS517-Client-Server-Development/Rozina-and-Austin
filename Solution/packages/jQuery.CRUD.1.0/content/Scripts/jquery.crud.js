...
/* Examples
    Disclaimer: use the minified version in production

    if the service endpoint is /books

    READ:
    $.Read("books", OnSuccess); // read all books
    $.Read("books", OnSuccess, { cache: true }); // alter the default settings 
    $.Read("books?genre={genre}", { genre:"Romance" }, OnSuccess); // all books within a specific genre (token replacement)
    $.Read("books/{id}", { id: "bk101" }, OnSuccess); // find by id
    ...
    var book = {    
        ...
        $.Create("books", OnSuccess, { data:book }); // create new book
        $.Create("books", book, OnSuccess); // create new book
        $.Create("books", { data:book }, OnSuccess); // create new book
    ...
    var book = {    
      ...
      $.Update("books/{id}", {id: "bk114", data: book}, OnSuccess); // update book bk114
      $.Update("books/{id}", {id: "bk114"}, OnSuccess, {data: book, cache:false}); // update book bk114 and alter default settings


    DELETE:
    $.Delete("books/{id}", { id: "bk114" }); // delete book bk114
...
(function ($, undefined) {
    ...
    $.extend({
        ...
        Create: function (uri, data, callback, options) {
            ...
            /// <param name="uri">[String] the uri template (required)</param>
            /// <param name="data">[Object|Function] token replacements and/or JSON object OR the $.ajax success callback (optional)</param>
            /// <param name="callback">[Object|Function] the $.ajax success callback OR the $.ajax settings (optional)</param>
            /// <param name="options">[Object] the $.ajax settings (optional)</param>
